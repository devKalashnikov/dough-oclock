...
const ENGINE = (() => {
  ...
  function poolishWaterTemp(T) {
    const target = T >= 26 ? 24 : T >= 20 ? 25 : 26;
    return Math.round(clamp(2 * target - T, 16, 35));
  }
  ...
  function planRT({ inp, env, EQR, L, ready, caps, pre, trw, warnings }) {
    ...
    steps.push(S(best.poolStart, '🥣', 'active', 'Make the poolish', 0.1, `Yeast first: suspend it in 30 g of the water at 38°C (finger-warm, never hot), water only, 10 min. Then the rest of the water at about ${poolishWaterTemp(T)}°C, the honey, the yeast slurry, then the maida; mix to a thick batter. Never dry yeast straight into cold water: it leaks and stalls. Loose cover on the counter for ${fmtH(pre.warm)}.`, 'Small bubbles by 45 min; by the end: domed, bubbling all over, centre just starting to flatten. If that happens early, go to the fridge early.'));
  ...
  function planS({ inp, env, EQR, L, ready, preS, warnings }) {
    ...
    steps.push(S(start, '🥣', 'active', 'Make the same-day poolish', 0.1, `Yeast first: suspend it in 30 g of the water at 38°C (finger-warm, never hot), water only, 10 min. Then the rest of the water at about ${poolishWaterTemp(T)}°C, the yeast slurry, then the maida; mix to a thick batter. No honey today. Loose cover on the counter for ${fmtH(preS.warm)}.`, 'Use it when it has at least doubled, is bubbling all over and the dome is just starting to flatten. If it gets there early, mix early.'));
