...
const ENGINE = (() => {
  ...
  function waterTemp(plan, T, Tf, rewarm, sameDay) {
    ...
    const notes = [];
    if (raw < 1) notes.push('The maths wants water below freezing: replace 20% of it (by weight) with crushed ice added with the first water, and chill the flour and the bowl 30 min in the fridge. Cold flour drinks slower, so expect the first mix to take a few minutes longer.');
  ...
  function build(inp) {
    ...
    rec.waterTemp = wtr; rec.poolishWaterTemp = plan === 'E' ? null : (out.poolishWater === 'room' ? Tm : out.poolishWater);
  ...
  function planRT({ inp, env, EQR, L, ready, caps, pre, trw, warnings }) {
    ...
    steps.push(S(best.poolStart, '🥣', 'active', 'Make the poolish', 0.1, `Whisk the yeast and honey into the water, add the maida, mix to a thick batter. Loose cover on the counter for ${fmtH(pre.warm)}.`, 'Small bubbles by 45 min; by the end: domed, bubbling all over, centre just starting to flatten. If that happens early, go to the fridge early.'));
    ...
    steps.push(S(mixStart, '🥣', 'active', `Mix the final dough (target dough temp ${pr.ddt}°C)`, K.mixH, `Water first: half of the water into the bowl (dissolve the salt in the other half and set it aside), then all the poolish, then the flours. Speed 1 for 2–3 min until no dry flour is left: a rough, tight ball. Cover and rest 15–20 min so the maida drinks. Speed 2 for 6–8 min until it gathers on the hook and pulls clean from the bowl. Second water: the salty half in 3–4 small additions on speed 2, waiting until each has vanished and the dough gathers again (8–10 min). Oil last, 2 min. Stop when smooth and satiny even if it fails the windowpane: fermentation finishes the gluten.`, 'Speed 2 is where a home mixer kneads; speed 1 only stirs. A hook spinning through sloppy dough that never gathers means too much water too early or too little dough for the bowl: hold the second water back, scrape down, give it 5 more min. Strandy or tearing? Stop, cover 15 min, then 3 min on speed 2. Dough above 27°C? Finish with three sets of stretch-and-folds instead. Never add flour.'));
  ...
  function planS({ inp, env, EQR, L, ready, preS, warnings }) {
    ...
    steps.push(S(start, '🥣', 'active', 'Make the same-day poolish', 0.1, `Whisk the yeast into the water, add the maida, mix to a thick batter. Loose cover on the counter for ${fmtH(preS.warm)}. No honey today.`, 'Use it when it has at least doubled, is bubbling all over and the dome is just starting to flatten. If it gets there early, mix early.'));
  ...
  function planE({ inp, env, L, ready, warnings }) {
    ...
    steps.push(S(mixStart, '🥣', 'active', 'Mix the dough now — no poolish (target dough temp 26°C)', K.mixH, `Dissolve the yeast in 50 g of the water at 32°C first. Water first: 80% of the water (salt dissolved in the rest, set aside), the yeast water, then the flours. Speed 1 for 2–3 min to a rough ball, rest 10 min, then speed 2 for 6–8 min until it gathers on the hook. Add the salty water in 2–3 small additions on speed 2, waiting for each to vanish (5 min). Oil last, 2 min. Hydration capped at 65%.`, 'Speed 2 is where a home mixer kneads; speed 1 only stirs. A hook spinning through sloppy dough that never gathers means too much water too early or too little dough for the bowl: hold the second water back, scrape down, give it 5 more min. Strandy or tearing? Stop, cover 15 min, then 3 min on speed 2. Dough above 27°C? Finish with three sets of stretch-and-folds instead. Never add flour.'));
  ...
  function planCold({ inp, env, EQR, L, ready, caps, pre, trw, hhs, warnings, minHb = 8, minHp = 6, relaxed = false }) {
    ...
    steps.push(S(best.poolStart, '🥣', 'active', 'Make the poolish', 0.1, `Whisk the yeast and honey into the water, add the maida, mix to a thick batter. Loose cover on the counter for ${fmtH(pre.warm)}.`, 'By the end: domed, bubbling all over, centre just starting to flatten. Early? Fridge early.'));
    steps.push(S(add(best.poolStart, 0.1), '🫧', 'wait', `Poolish wakes up on the counter (${T}°C)`, pre.warm - 0.1));
    ...
    steps.push(S(tp.mixStart, '🥣', 'active', 'Mix the final dough (target dough temp 20°C — it\'s going to the fridge)', K.mixH, `Water first: half of the cold water into the bowl (dissolve the salt in the other half and set it aside), then all the poolish, then the flours. Speed 1 for 2–3 min until no dry flour is left: a rough, tight ball. Cover and rest 15–20 min so the maida drinks. Speed 2 for 6–8 min until it gathers on the hook and pulls clean from the bowl. Second water: the salty half in 3–4 small additions on speed 2, waiting until each has vanished and the dough gathers again (8–10 min). Oil last, 2 min. Stop when smooth and satiny even if it fails the windowpane: fermentation finishes the gluten.`, 'Speed 2 is where a home mixer kneads; speed 1 only stirs. A hook spinning through sloppy dough that never gathers means too much water too early or too little dough for the bowl: hold the second water back, scrape down, give it 5 more min. Strandy or tearing? Stop, cover 15 min, then 3 min on speed 2. Dough above 27°C? Finish with three sets of stretch-and-folds instead. Never add flour.'));
