...
const ENGINE = (() => {
  ...
  function build(inp) {
    ...
    if (Tm >= 31) warnings.push({ level: 'danger', text: `Hot kitchen (${T}°C): the ready window is only ~30 min. If an AC room at 24–27°C exists, enter that temperature instead. Ice water, hydration ≤ 65%, and bring the balls to readiness in waves of 4.` });
    ...
    if (Tm >= 30 && inp.eatNow >= 5) { const rest = inp.eatNow - 4, bakeMin = Math.round(inp.eatNow * 3.5);
      warnings.push({ level: 'info', text: plan === 'COLD'
        ? `Two waves: at ${T}°C a proofed ball is at its best for only ~30 min, and ${inp.eatNow} pizzas take ~${bakeMin} min to stretch and bake. Take 4 balls out of the fridge at the scheduled time and the other ${rest} about 20 min later, so the last pizzas are not stretched past their peak.`
        : `Two waves: at ${T}°C a proofed ball is at its best for only ~30 min, and ${inp.eatNow} pizzas take ~${bakeMin} min to stretch and bake. Proof 4 balls as scheduled. Put the other ${rest} in the fridge 30 min after balling and bring them back to the counter 1 h later; they will be ready about 30 min after the first four.` }); }
  ...
  function planCold({ inp, env, EQR, L, ready, caps, pre, trw, hhs, warnings, minHb = 8, minHp = 6, relaxed = false }) {
    ...
    steps.push(S(tp.fridgeOut, '🌡️', 'wait', `Balls out to finish proofing at ${T}°C`, tp.tt, `About ${fmtH(tp.tt)}: they must both finish rising and warm up inside (cold dough tears and blisters at 500°C). ${inp.eatNow >= 5 && T >= 28 ? `Two waves: 4 balls now, the other ${inp.eatNow - 4} about 20 min later.` : ''}`));
